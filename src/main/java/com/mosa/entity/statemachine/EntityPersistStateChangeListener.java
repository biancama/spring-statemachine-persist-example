...
@Component
public class EntityPersistStateChangeListener implements CustomFactoryPersistStateMachineHandler.CustomPersistStateChangeListener {
  ...
  @Override
  public void onPersist(State<EntityStates, EntityEvents> state, Message<EntityEvents> message, Transition<EntityStates, EntityEvents> transition, StateMachine<EntityStates, EntityEvents> stateMachine) {
    if (message != null && message.getHeaders().containsKey(EntityConstants.entityHeader)) {
      Entity entity = message.getHeaders().get(EntityConstants.entityHeader, Entity.class);
